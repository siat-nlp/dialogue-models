...
def masked_binary_cross_entropy(logits, target, length, use_cuda=True):
    '''
...
def masked_coverage_loss(coverage, attention, length, use_cuda=True):
    if use_cuda:
        length = Variable(torch.LongTensor(length)).cuda()
    else:
        length = Variable(torch.LongTensor(length))
...
def masked_cross_entropy_RL(logits, target, length, q=1.0, use_cuda=True):
    """
    Args:
        logits: A Variable containing a FloatTensor of size
            (batch, max_len, num_classes) which contains the
    ...
    Returns:
        loss: An average loss value masked by the length.
    """
    if isinstance(length, torch.cuda.LongTensor or isinstance(length, torch.LongTensor)):
